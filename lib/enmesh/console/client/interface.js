...
cluster.interface = {
    refresh: function() {
        ...
        switch (parts[0]) {
            case 'instance':
                ...
                $('#instance .type').text(parts[1]);
                $('#instance .name').text(parts[2]);
                cluster.data.watch(parts[1], parts[2]);
                break;
            case 'instances':
    ...
    update: {
        ...
        watching: function() {
            ...
            var data = cluster.data.watching.data || {};
            console.log(data);
            Object.keys(data).sort().forEach(function(name, index) {
                var partition = cluster.data.partitions[data[name].partition];
                var entry = $.tmpl('<tr class="data" id="data-${name}"><td>${partition}</td><td><span title="${partition} - ${owner}">${name}</span></td><td>${value}</td></tr>', {name: name, value: JSON.stringify(data[name].value), partition: partition.id, owner: partition.owner});
